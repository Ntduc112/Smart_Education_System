...
export async function sendRegisterOtpEmail(email: string, code: string) {
    const { data, error } = await getResend().emails.send({
        from: process.env.RESEND_FROM_EMAIL ?? "Learnust <noreply@learnust.dev>",
        to: email,
        subject: "Mã xác thực đăng ký tài khoản – Learnust",
        html: `
<!DOCTYPE html>
<html>
<body style="margin:0;padding:0;background:#f4f6fb;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:40px 20px">
    <tr><td align="center">
      <table width="480" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08)">
        ...
        <tr>
          <td style="padding:40px">
            <h2 style="margin:0 0 12px;font-size:18px;font-weight:600;color:#181d26">Xác thực email đăng ký</h2>
            <p style="margin:0 0 28px;color:rgba(4,14,32,.55);font-size:14px;line-height:1.6">
              Cảm ơn bạn đã đăng ký Learnust.<br/>
              Nhập mã bên dưới để hoàn tất tạo tài khoản. Mã có hiệu lực trong <strong>10 phút</strong>.
            </p>
            <div style="text-align:center;margin:0 0 28px">
              <div style="display:inline-block;background:#f0f4fb;border:1px solid #d0dff7;border-radius:12px;padding:20px 40px">
                <span style="font-size:36px;font-weight:700;letter-spacing:12px;color:#1b61c9">${code}</span>
            ...
            <p style="margin:0;color:rgba(4,14,32,.4);font-size:12px;line-height:1.6">
              Nếu bạn không đăng ký tài khoản Learnust, hãy bỏ qua email này.<br/>
              Sẽ <strong>không có tài khoản nào</strong> được tạo với email của bạn.
            </p>
          </td>
        </tr>
        <tr>
...
export async function sendAssistantWelcomeEmail(input: {
    email: string;
    name: string;
    password: string;
    courseTitle: string;
      ...
      <table width="480" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08)">
        ...
        <tr>
          <td style="padding:40px">
            ...
            <p style="margin:0 0 28px;color:rgba(4,14,32,.55);font-size:14px;line-height:1.6">
              Giáo viên <strong>${input.teacherName}</strong> đã tạo tài khoản trợ giảng cho bạn
              trong khóa học <strong>“${input.courseTitle}”</strong> trên Learnust.
            </p>
            <div style="background:#f0f4fb;border:1px solid #d0dff7;border-radius:12px;padding:20px 24px;margin:0 0 28px">
              <p style="margin:0 0 8px;font-size:14px;color:#181d26"><strong>Email:</strong> ${input.email}</p>
              <p style="margin:0;font-size:14px;color:#181d26"><strong>Mật khẩu tạm:</strong> ${input.password}</p>
            </div>
            <div style="text-align:center;margin:0 0 28px">
