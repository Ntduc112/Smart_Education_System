    ...
    <div className="min-h-screen bg-[#EFF5FE] text-[#181d26]">
      ...
      <main className="mx-auto max-w-6xl px-6 py-9">
        ...
        <section>
          ...
          ) : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{memberships.map((item) => <CourseCard key={item.id} item={item} />)}</div>}
        </section>
      </main>
    </div>
  );
...
function CourseCard({ item }: { item: AssistantMembership }) {
  return <article className="overflow-hidden rounded-3xl border border-[#DCE6F4] bg-white shadow-[rgba(80,60,20,0.04)_0px_8px_24px]">
    <div className="relative aspect-[16/7] overflow-hidden bg-[#DDE9F8]"><Image src={item.course.thumbnail} alt="" fill sizes="(max-width: 768px) 100vw, 420px" className="object-cover" /></div>
    <div className="p-5">
