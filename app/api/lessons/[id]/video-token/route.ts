...
export async function GET(
    ...
    const { instructor_id, enrollments, collaborators } = lesson.chapter.course;
    const hasAccess = instructor_id === userId || lesson.is_free || enrollments.length > 0 || collaborators.length > 0;

    if (!hasAccess) {
        return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    }

    const videoKey = lesson.video_url.slice(3);
